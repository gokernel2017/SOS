...
//		sprintk (buf, "%s: %d", "FPS", value);
    Console_Format ("Time: %d", value);
/*
    Console_Format ("Time: %d:%d:%d | %d | FPS %d",
...
void Console_WriteUInt32(unsigned int num, int digits, char padding, int base, char suppressZero) {
   if (digits == 0) {
      return;
   }

   Console_WriteUInt32(num / base, digits - 1, padding, base, 1);

   if (num == 0 && suppressZero) {
      if (padding) {
	 video_putchar(padding);
      }
   } else {
      unsigned char digit = num % base;
      video_putchar(digit >= 10 ? digit - 10 + 'A' : digit + '0');
   }
}
...
void Console_FormatV(const char **args) {
   ...
   while ((c = *(fmt++))) {
      ...
      while ((c = *(fmt++))) {
         ...
         if (c == 'c') {
            video_putchar((char)(unsigned int) *(args++));
            break;
         }
         ...
         if (base) {
             ...
             */
            if (isSigned && 0 > (int)value) {
               video_putchar('-');
               width--;
               value = -value;
            }

            Console_WriteUInt32(value, width, padding, base, 0);
            break;
         }
